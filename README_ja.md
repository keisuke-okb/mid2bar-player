...
３行ブロックの１行目（画面上からA～D行としたときのB行に配置）の行末を全角スペース（U+3000）にすると、B行とC行を左側に揃え、D行を右側に配置できます。例えば、次のようにタイムタグに挟まれた歌詞文字列の末尾が全角スペースになる場合が対象です（ルビは省略しています）。

```
[00:35:29]シャ[00:35:45]イ[00:35:61]ニ[00:35:76]ン[00:35:94]グ[00:36:09]ス[00:36:24]ター[00:36:58]綴[00:37:30]れ[00:37:49]ば　[00:37:94]
```

この配置では、末尾の全角スペースは文字位置の計算から除外されます。B行が長く、実際の文字の右端がD行の右端を越える場合は、C行・D行の左右配置を維持したまま、B行だけを左端側へ配置します。
...
| 項目 | デフォルト値 | 説明 |
|------|-------------|------|
| `RANGE_GAUGE_MAP_POS` | [1008, 354] | 音域ゲージのマップの表示位置 [x, y] |
| `RANGE_GAUGE_MAP_W` | 626 | 音域ゲージマップの幅（ピクセル） |
| `RANGE_GAUGE_MAP_H` | 23 | 音域ゲージマップの高さ（ピクセル） |
| `RANGE_GAUGE_MAP_PAGE_MAX_RADIUS` | 100 | 各ページアイコンの角丸（角の丸み）の最大半径 (px) |
| `RANGE_GAUGE_MAP_PAGE_MAX_WIDTH` | 30 | ページアイコンが横長になりすぎるのを防ぐ最大幅 (px) |
| `RANGE_GAUGE_MAP_PAGE_MARGIN` | 3 | ページアイコン同士の間の余白幅 (px) |
| `RANGE_GAUGE_MAP_PAGE_PAST_ALPHA` | 80 | 通過済み（演奏済み）ページの不透明度 (0-255) |
| `RANGE_GAUGE_MAP_BLANK_COLOR` | (100, 100, 100) | ノート（音符）が存在しない空ページの表示色 [RGB] |
| `RANGE_GAUGE_MAP_BLANK_S` | 3.0 | 空ページとみなす音程バーのない区間の長さのしきい値 (秒) |
| `RANGE_GAUGE_MAP_BLINK_COLOR` | (200, 200, 200) | 現在のページ（アクティブ）ページの点滅枠の色 [RGB] |
| `RANGE_GAUGE_MAP_BLINK_ON_S` | 1.0 | 点滅処理における点灯状態（ON）の継続時間 (秒) |
| `RANGE_GAUGE_MAP_BLINK_OFF_S` | 0.2 | 点滅処理における消灯状態（OFF）の継続時間 (秒) |
| `RANGE_GAUGE_MAP_BLINK_BORDER` | 5 | 選択中ページの点滅枠の太さ (px) |
